...
- **Source:**  Un atacante o bot automatizado intenta degradar la disponibilidad del sistema. Puede provenir de una sola IP o de múltiples orígenes coordinados.
...
- **Risk (Availability):** Degradación del rendimiento o caída del servicio para usuarios legítimos por agotamiento de CPU/memoria/conexiones.
...
**un-wheels-frontend:** El Frontend web del sistema y su punto de entrada público
...
**Zona estable (≤ 100 VU)** El sistema opera con un comportamiento óptimo y predecible:
- **Latencia promedio:** < 64ms, en todos los puntos (1, 10 y 50 VUs)
- **Tasa de errores:** 0%, todos los puntos marcados en verde.
- **Comportamiento:** La curva es prácticamente plana, lo que indica que el sistema maneja la carga sin degradación perceptible.
- **Conclusión:** El sistema tiene capacidad ociosa y responde de forma consistente.

**Knee point (100 VUs)** Este es el punto de inflexión crítico donde el sistema comienza a colapsar:

- **Latencia promedio:** ~3,000ms, un incremento de +5,573% respecto a la zona estable
- **Tasa de errores:** 12%, aparecen los primeros fallos (punto rojo)
- **Comportamiento:** La curva sube de forma abrupta entre 50 y 100 VUs, lo que indica que el sistema superó su capacidad de procesamiento sostenible
- **Conclusión:** Este es el límite real de capacidad del sistema. Superar los 100 VUs no es viable en producción. Cualquier pico de tráfico por encima de este umbral desencadena una degradación severa

**Colapso total (≥ 500 VUs)** El sistema entra en un estado de fallo irrecuperable:

- **Latencia promedio:** ~29,879ms en 500 VUs y se mantiene igual en 2,000 VUs
- **Tasa de errores:** 100%, ninguna petición se completa exitosamente
- **Comportamiento:** La latencia se "techa" (plateau) cerca de los 30,000ms, lo que sugiere que el sistema está descartando o expirando todas las solicitudes con un timeout fijo. Agregar más usuarios no cambia el tiempo de respuesta porque el sistema ya no procesa nada
- **Conclusión:** La aplicación está completamente saturada. El plateau indica un timeout hardcodeado (~30s), no capacidad real de respuesta

### Resumen — Prueba de Carga

| Zona | VUs | Latencia Promedio | Tasa de Errores | Estado |
|---|---|---|---|---|
| Estable | ≤ 50 | < 64ms | 0% | Producción segura |
| Knee Point | 100 | ~3,000ms (+5,573%) | 12% | Límite crítico |
| Colapso Total | ≥ 500 | ~29,879ms | 100% | Inoperativo |
