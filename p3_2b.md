...
| **Service Discovery Pattern**       | DNS estable `loggueo-service` → HAProxy (`loggueo-lb`) con pool de 3 réplicas. `option httpchk GET /readyz` (cada 5 s) es la **única fuente de verdad** para enrutamiento; excluye instancias caídas antes de peticiones de usuario. Heartbeat (`healthmon`) observa servicios sin LB, sin duplicar checks a réplicas auth. |
| **Load Balancer Pattern**           | HAProxy balancea `loggueo_1/2/3` con Round-Robin (configurable a Least-Conn o Source IP). El api-gateway resuelve una sola URL; el LB descubre y excluye réplicas unhealthy vía httpchk.                                                                                                    |
