...
Todos los tests apuntan a `POST /api/auth/login` (y, en el Test 5, a la sesión completa) sobre `https://localhost:8080` usando k6. El check de éxito es `status 201` en login y `status 200` en los demás endpoints. El rate-limiting de nginx fue deshabilitado durante las pruebas para medir la capacidad real del stack de aplicación.
...
El sistema se comporta establemente bajo carga mínima. En 30 segundos se procesaron 25 peticiones sin un solo fallo. La latencia media de **205 ms** representa el costo real de la cadena completa: WAF → rp-web → frontend SSR → API Gateway → loggueo-service → PostgreSQL. El timing breakdown confirma que prácticamente el 100 % del tiempo es TTFB (204,92 ms de 205,21 ms totales); el overhead de sending (0,22 ms) y receiving (0,06 ms) es despreciable.

El spread entre p50 (211 ms) y p95 (216 ms) es de solo **5 ms**, lo que indica un comportamiento muy predecible: el cuello de botella es el costo fijo de bcrypt (~200 ms) más el round-trip a PostgreSQL, sin varianza apreciable. El throughput efectivo fue de **0,86 req/s**, limitado por el `sleep(1)` del script de test.
...
**0 % de fallos** con 50 usuarios concurrentes. La latencia media subió de 205 ms a **296 ms** (+44 %), y el p95 pasó de 216 ms a **471 ms**. La cola del Max en 1.259 ms se explica por el spike visible en el gráfico al inicio de la prueba: el sistema necesita establecer 50 conexiones simultáneas al pool de PostgreSQL en frío, lo que genera un pico transitorio que luego se estabiliza.

Después del warm-up inicial, el sistema se estabiliza en el rango 200–400 ms de forma consistente. Los picos periódicos observados en el gráfico están probablemente asociados a ciclos de GC del intérprete Python de FastAPI o momentos de saturación momentánea del pool de conexiones. El throughput alcanzó **~39 req/s** con 0 % de error rate, lo que confirma que la zona de operación segura del servicio de login se sitúa en torno a este nivel de concurrencia.

---

### 5.3 Test 3 - Stress Ramp-up (1→50→200→500→2000 VUs · 3 min)
...
![Performance Test](./imgs/performance_report/performance_report-19.png)

El test ejecutó una rampa escalonada de 6 etapas (1→50→200→500→2000→0 VUs, 30 s cada una). De 4.756 peticiones totales, **1.065 respondieron HTTP 201** (22,4 %) y **3.691 respondieron HTTP 500** (77,6 %). Las 1.065 peticiones exitosas tuvieron una latencia media de 776 ms, significativamente superior a los 296 ms del Test 2, lo que evidencia que incluso las peticiones que sí se procesan sufren contención severa de recursos.

El colapso ocurre durante la etapa de 200 VUs (t ≈ 60–90 s). La convergencia de mediana, p90, p95 y máximo en exactamente ~30.000 ms —el timeout configurado en k6— confirma que las peticiones no están siendo rechazadas activamente sino **quedando en cola a la espera de un recurso que nunca llega**. Esto es estructuralmente peor que un rechazo rápido: mantiene conexiones abiertas y agota recursos de forma progresiva. Los fallos son todos HTTP 500, no 503 ni 429, lo que confirma que el cuello de botella está dentro de la aplicación (agotamiento del pool de conexiones de PostgreSQL en el loggueo-service) y no en el WAF ni en Nginx.

| Métrica | 1 VU | 50 VUs | Ramp-up |
|---|---|---|---|
| Avg | 205 ms | 296 ms | 23.448 ms |
| p95 | 216 ms | 471 ms | 30.011 ms |
| Failure rate | 0 % | 0 % | 77,6 % |
| Throughput | 0,86 req/s | ~39 req/s | ~23 req/s (con fallos) |

---

### 5.4 Test 5 - Full Session Journey (1→10→50→100→500→2000 VUs · 3,75 min)

![Performance Test](./imgs/performance_report/performance_report-21.png)
![Performance Test](./imgs/performance_report/performance_report-23.png)
![Performance Test](./imgs/performance_report/performance_report-25.png)

El Test 5 simula una sesión de usuario completa en cuatro pasos encadenados: **login → /me → search/routes → logout**. Cada paso está etiquetado individualmente en k6, lo que permite medir la latencia y el error rate por endpoint. La rampa de carga es idéntica a la del Test 5 del knee point (1→10→50→100→500→2000 VUs en etapas de 30 s).

De 14.221 peticiones totales, la tasa global de fallos fue del **~40 %**, pero el análisis por paso revela un patrón sumamente asimétrico:

| Paso | Avg | p95 | Fallos | Servicio |
|---|---|---|---|---|
| `POST /api/auth/login` | 13.934 ms | 30.216 ms | **69 %** | loggueo-service (Python/FastAPI) |
| `GET /api/auth/me` | 3.047 ms | 15.100 ms | **12,3 %** | loggueo-service (Python/FastAPI) |
| `GET /api/search/routes` | 57 ms | 241 ms | **0 %** | search-service (Go) |
| `POST /api/auth/logout` | 35 ms | 130 ms | **0 %** | stateless (cookie clear) |

El diagnóstico es claro: **el search-service en Go y el logout son completamente resilientes bajo toda la carga aplicada**. El único punto de fallo es el loggueo-service de Python, cuya saturación contamina incluso el endpoint `/me` (que reutiliza el mismo proceso) aunque en menor medida que el login. Escalar horizontalmente solo el loggueo-service —sin tocar el resto del stack— sería suficiente para mejorar la capacidad del sistema de forma significativa.
