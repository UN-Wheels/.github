...
| **Heartbeat Pattern** (equipo)      | `healthmon` hace polling cada 15s con umbral de 3 fallos; Docker healthchecks y heartbeat AMQP en RabbitMQ registran latidos. Solo observa y loguea (`UNHEALTHY`/`RECOVERED`); no enruta tráfico.                                                                                  |
| **Service Discovery Pattern**       | DNS estable `loggueo-service` → nginx LB (`loggueo-lb`) con pool de 3 réplicas. `max_fails=3 fail_timeout=30s` retira instancias caídas; `/readyz` en auth verifica PostgreSQL antes de recibir tráfico. Heartbeat y SD son capas separadas.                                        |
| **Load Balancer Pattern**           | Nginx interno balancea `loggueo_1/2/3` con Round-Robin (configurable a Least-Conn o IP-Hash). El api-gateway resuelve una sola URL; el LB descubre y excluye réplicas unhealthy.                                                                                                    |
| **Despliegue en Nodo Único**        | Para este prototipo, todos los contenedores se ejecutan en un único Docker Host mediante Docker Compose. La zonificación de la arquitectura (Edge / Web / Gateway / Services / Data / loggueo-internal) está diseñada para migrar a orquestadores como Kubernetes con mínimo refactoring. |
