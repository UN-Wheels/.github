...
![Vista de Despliegue](./imgs/Desplieguee.png)

</div>

#### Descripción de Elementos Arquitectónicos y Relaciones

El sistema se despliega en un clúster distribuido multi-nodo utilizando Docker Swarm sobre una red local. La infraestructura se compone de dos nodos físicos: un **Nodo Manager**, que centraliza el plano de control, la suite perimetral y la mayoría de los microservicios; y un **Nodo Worker**, configurado como nodo de computación dedicado para la alta disponibilidad del core de negocio. La arquitectura aplica Network Segmentation mediante redes virtuales superpuestas (Overlay Networks de Swarm), organizando el tráfico en siete zonas lógicas: Zona Móvil, Edge, Web, Gateway, Services (con sub-zona loggueo-internal), Broker & Cache, y Egress (acceso controlado a bases de datos en la nube).
...
| unwheels-search        | `route-search-service`  | Go / Gin (contenedor Linux) | 6000             | 1 Instancia (Fija en Nodo Manager)                                                      |

###### Loggueo-Internal Zone (sub-zona dentro de Services Zone)

Red aislada que contiene exclusivamente el clúster de autenticación. El `api-gateway` no accede directamente a las réplicas; todo el tráfico pasa por `loggueo-lb` (HAProxy), que expone el nombre DNS estable `loggueo-service`.

| Elemento de Despliegue | Contenedor   | Runtime                        | Puerto (interno) | Descripción                                                                                                              |
| ---------------------- | ------------ | ------------------------------ | ---------------- | ------------------------------------------------------------------------------------------------------------------------ |
| loggueo-lb             | `loggueo-lb` | HAProxy 2.x                    | 8000             | Balanceador de carga para las 3 réplicas de autenticación. Expone el nombre lógico `loggueo-service` a la Services Zone. |
| loggueo_1              | `loggueo_1`  | Python 3.x / Uvicorn (FastAPI) | 8000             | Réplica 1 del servicio de autenticación. Replication Pattern: lecturas → réplica Neon, escrituras → primario.            |
| loggueo_2              | `loggueo_2`  | Python 3.x / Uvicorn (FastAPI) | 8000             | Réplica 2 del servicio de autenticación. Replication Pattern: lecturas → réplica Neon, escrituras → primario.            |
| loggueo_3              | `loggueo_3`  | Python 3.x / Uvicorn (FastAPI) | 8000             | Réplica 3 del servicio de autenticación. Replication Pattern: lecturas → réplica Neon, escrituras → primario.            |

##### Broker & Cache Zone (red interna Docker)

Zona dedicada a la mensajería asíncrona y la caché en memoria. Sin acceso a internet; los microservicios la alcanzan desde la red `data`.

| Elemento de Despliegue | Contenedor     | Runtime                   | Puerto (interno) | Descripción                                                                                                  |
| ---------------------- | -------------- | ------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------ |
| unwheels-rabbitmq      | `event-broker` | RabbitMQ 3.13 (Erlang VM) | 5672 (AMQP)      | Broker de eventos para comunicación asíncrona entre microservicios (chat, rutas, notificaciones).            |
| unwheels-search-cache  | `search-redis` | Redis 7 (Alpine)          | 6379             | Caché en memoria (Cache-Aside) para resultados del servicio de búsqueda de rutas. TTL de 120 s, 128 MB máx. |

##### Egress Zone (acceso controlado a bases de datos en la nube)

Red no-interna (`unwheels-egress`) que proporciona salida a internet únicamente a los servicios que requieren conectividad con proveedores de base de datos en la nube. El resto de zonas no tiene acceso a internet. Se subdivide lógicamente en dos proveedores cloud:

**Neon Cloud (PostgreSQL serverless)**

| Base de Datos       | Endpoint Neon (pooler IPv4)                                       | Puerto | Consumidor                                              |
| ------------------- | ----------------------------------------------------------------- | ------ | ------------------------------------------------------- |
| login-db (primario) | `ep-small-tooth-apkq7bfj-pooler.c-7.us-east-1.aws.neon.tech`     | 5432   | `loggueo_1/2/3` (escrituras y lecturas de fallback)     |
| login-db (réplica)  | `ep-empty-firefly-apmpo2wm-pooler.c-7.us-east-1.aws.neon.tech`   | 5432   | `loggueo_1/2/3` (lecturas — intento preferente)         |
| search-db           | `ep-holy-dream-app9fr6m-pooler.c-7.us-east-1.aws.neon.tech`      | 5432   | `route-search-service`                                  |

Todos los endpoints usan el sufijo `-pooler` para garantizar conectividad IPv4 desde contenedores Docker (las redes bridge son IPv4; los endpoints directos de Neon solo responden a IPv6).

**MongoDB Atlas**

| Base de Datos    | Puerto | Consumidor              |
| ---------------- | ------ | ----------------------- |
| chat-db          | 27017  | `chat-service`          |
| notifications-db | 27017  | `notifications-service` |
| routes-db        | 27017  | `routes-service`        |
...
- Todos los contenedores de las zonas internas (Web, Gateway, Services, Broker & Cache y loggueo-internal) son redes Docker `internal: true`; ninguno tiene salida a internet.
- El `api-gateway` resuelve el nombre lógico `loggueo-service` hacia `loggueo-lb` (HAProxy), que distribuye las peticiones entre `loggueo_1`, `loggueo_2` y `loggueo_3` mediante Round-Robin con health checks activos (`httpchk GET /readyz` cada 5 s).
- Las réplicas `loggueo_1/2/3` están conectadas a la red `egress` para alcanzar los endpoints de Neon Cloud. Las escrituras (registro, login) van siempre al nodo primario; las lecturas intentan primero la réplica y caen automáticamente al primario ante cualquier `OperationalError`.
- `route-search-service` usa la red `egress` para su base de datos PostgreSQL en Neon Cloud, y la red `data` para acceder a `search-redis`.
- `chat-service`, `notifications-service` y `routes-service` acceden a sus respectivas bases de datos en MongoDB Atlas a través de la red `egress`.
- Los microservicios que producen o consumen eventos acceden a `event-broker` a través de la red `data`.
- Los datos persistentes del broker se almacenan en el volumen Docker `rabbitmq_data`. Las bases de datos de aplicación residen en la nube (Neon y Atlas) sin volúmenes locales.
...
| **Aislamiento de Red Interna**      | Las zonas internas (Web, Gateway, Services, Broker & Cache, loggueo-internal) son redes Docker `internal: true`, sin salida a internet. Solo la red `egress` (`internal: false`) permite conectividad saliente controlada hacia los proveedores cloud.                                                                                                                                                                                                     |
| **Database per Service**            | Cada microservicio tiene su propia base de datos en la nube (Neon o Atlas). Ningún par de servicios comparte base de datos, garantizando acoplamiento débil en la capa de datos.                                                                                                                                                                                                                                                                          |
| **Replication Pattern**             | `loggueo_service` implementa un split lectura/escritura sobre los dos endpoints de Neon: escrituras van siempre al nodo primario (`engine_primary`) y lecturas intentan primero la réplica (`engine_replica`). Si la réplica no responde, el sistema cae automáticamente al primario sin interrumpir el servicio (failover transparente).                                                                                                                   |
| **Heartbeat Pattern** (equipo)      | `healthmon` hace polling cada 15 s con umbral de 3 fallos a servicios **sin LB propio** (chat, routes, notifications, search, liveness del LB). No repite httpchk de réplicas auth — eso lo hace HAProxy. Docker healthcheck usa `/` (liveness) en réplicas.                                                                                                                                                                                              |
...
| **Despliegue Multi-nodo (Clúster)** | La infraestructura lógica se distribuye en un clúster multi-nodo gestionado por Docker Swarm sobre una red local. La suite perimetral, el API Gateway y los servicios de soporte se anclan al Nodo Manager. El microservicio crítico `routes-service` implementa el Cluster Pattern distribuyendo de forma asimétrica una de sus dos réplicas hacia el Nodo Worker, garantizando alta disponibilidad del core de negocio.                                   |
...
| **Balanceador de auth** | HAProxy (`loggueo-lb`) + 3 réplicas `loggueo_1/2/3` (4 workers uvicorn c/u) |
| **Búsqueda**            | `route-search-service` (Go) + Redis (`search-redis`) + PostGIS              |
| **Herramienta**         | k6 v2.0                                                                     |

> **Nota metodológica:** Durante las pruebas de estrés el host alcanzó **~95 % de uso de CPU** (1133 % / 1200 % en Docker Desktop), lo que provocó timeouts antes de completar rampas superiores a 300 VUs. La gráfica de rodilla del login se modeló a partir de los tramos medidos (baseline, load y buckets parciales) y del comportamiento arquitectónico conocido (bcrypt + pool PostgreSQL).
...
![Rodilla de rendimiento — Login](./imgs/performance_report/knee_point_login_simulado.png)
...
![Escenario 1 — Load Balancer Pattern](./imgs/performance_report/Load_balancer_Pattern.png)

</div>

**Nota:** El tiempo reportado en la medida de respuesta corresponde al comportamiento esperado del sistema bajo las condiciones del escenario; el patrón busca demostrar que el **Load Balancer** reparte la carga de autenticación entre réplicas antes de alcanzar saturación.
...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------- |
| **Maintain Multiple Copies of Computations**   | Tres réplicas de `loggueo_service` detrás de HAProxy; 4 workers uvicorn por réplica      | Load Balancer, Service Replication |
...
| ----------------------------------- | --------------------------------------- | -------------------------------------------------------------------- |
| **Load Balancer**                   | `loggueo-lb` (HAProxy, least-conn)      | Distribuye login entre 3 réplicas; health check activo `GET /readyz` |
| **Service Replication (Hot Spare)** | `loggueo_1/2/3`                         | Tres instancias activas; HAProxy excluye réplicas no listas          |
| **Cache-Aside**                     | `route-search-service` + `search-redis` | Consulta Redis antes de PostGIS; degradación elegante si Redis cae   |
...
**3. El loggueo-service es el cuello de botella.** La rodilla aparece ~300 VUs por el costo CPU de bcrypt. Mejoras puntuales: driver async (`asyncpg`), `create_login_log` en background y circuit breaker en el API Gateway.
...
![Escenario 1 — Replication Pattern (Hot Spare)](./imgs/replicationpattern2.png)
...
![Escenario 4 — Heartbeat Pattern](./imgs/reliability/scenario_heartbeat.jpeg)
...
**Prerrequisitos:** Docker Desktop ≥ 24.x, Git, [mkcert](https://github.com/FiloSottile/mkcert), puertos **8080**/**8081**.
...
for r in FrontEnd api-gateway loggueo_service chat-service routes-reservations-service \
  notifications-service search-service mobile security; do git clone https://github.com/UN-Wheels/$r.git; done
...
# 3. .env — FrontEnd/: JWT_SHARED_SECRET, FRONTEND_URL y NEXT_PUBLIC_API_URL → https://localhost:8080
