...
El sistema se despliega en un único host local (LAN PC) mediante Docker Compose. La infraestructura se organiza en contenedores Docker aislados por redes bridge internas, segmentadas en cinco zonas lógicas que implementan el patrón de Network Segmentation. El cliente móvil opera en una red separada (LAN-mobile) y se conecta al backend exclusivamente a través del punto de entrada expuesto del reverse proxy móvil.
...
| ---------------------- | ----------------------- | ------------------------------ | ---------------- |
| loggueo-lb             | `loggueo-lb`            | HAProxy                        | 8000             |
...
- Todos los contenedores de las zonas internas (Web, Gateway, Services y Data) se comunican a través de redes bridge Docker independientes mediante DNS interno basado en nombres de contenedor.
...
- El tráfico hacia `unwheels-auth` pasa primero por `loggueo-lb` (HAProxy), que balancea la carga entre las réplicas del servicio de autenticación.
...
| **Aislamiento de Red Interna**     | Redes bridge Docker independientes por zona (edge, web-zone, gateway-zone, services-zone, data-zone) aíslan los servicios internos. Los proxies de la Edge Zone son los únicos contenedores con puertos publicados al host; el resto solo hace `expose`.                  |
...
| **Heartbeat Pattern** (equipo)     | `healthmon` hace polling cada 15 s con umbral de 3 fallos a servicios **sin LB propio** (chat, routes, notifications, search, liveness del LB). No repite httpchk de réplicas auth — eso lo hace HAProxy. Docker healthcheck usa `/healthz` (liveness) en réplicas.      |
| **Service Discovery Pattern**      | DNS estable `loggueo-service` → HAProxy (`loggueo-lb`) con pool de réplicas. `option httpchk GET /readyz` (cada 5 s) es la **única fuente de verdad** para enrutamiento; excluye instancias caídas. Heartbeat observa servicios sin LB, sin duplicar checks a réplicas auth. |
| **Load Balancer Pattern**          | HAProxy (`loggueo-lb`) balancea las réplicas de `auth-service` con Round-Robin (configurable a Least-Conn o Source IP). El `api-gateway` resuelve una sola URL; el LB excluye réplicas unhealthy vía httpchk.                                                            |
