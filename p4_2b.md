...
**Prerrequisitos:** Docker Desktop ≥ 24.x, Git, [mkcert](https://github.com/FiloSottile/mkcert) y puertos **8080** / **8081** libres. Solo `rp-web` y `rp-mobile` exponen puertos al host; el resto del stack (SSR, gateway, BDs, RabbitMQ) queda en redes internas.

**Paso 1 — Clonar** (todos como hermanos; `docker-compose.yml` en `FrontEnd/`):
...
**Paso 2 — Certificados TLS** (no versionados; generar con mkcert):

```bash
mkcert -install
cd security/reverse-proxies && mkdir -p certs && cd certs
...
**Paso 3 — Variables de entorno** (solo `FrontEnd/.env` para el stack; `mobile/.env` desde `.env.example`):

```env
# FrontEnd/.env
JWT_SHARED_SECRET=dev_secret_change_me
FRONTEND_URL=https://localhost:8080
NEXT_PUBLIC_API_URL=https://localhost:8080

# mobile/.env
API_BASE_URL=https://127.0.0.1:8081/api
CHAT_SOCKET_URL=https://127.0.0.1:8081
```
...
| Servicio              | URL                                      |
| --------------------- | ---------------------------------------- |
| Web (SSR + WAF)       | https://localhost:8080                   |
| API canal web         | https://localhost:8080/api/auth/health   |
| API canal móvil       | https://localhost:8081/api/auth/health   |

`docker compose ps` debe mostrar únicamente `8080→443` (rp-web) y `8081→443` (rp-mobile).

**App móvil (Flutter):** `cd mobile && flutter pub get && flutter run`. Usar `https://10.0.2.2:8081/api` en emulador Android; en dispositivo físico, `adb reverse tcp:8081 tcp:8081` o la IP LAN de la PC. Instalar la CA de mkcert en el dispositivo si Flutter rechaza el certificado.
