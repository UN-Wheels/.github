...
![Vista en Capas](./imgs/capas4.jpeg)
...
| **Balanceo**               | `loggueo-lb`, `healthmon`                                                                        | HAProxy, Node.js                                    | Capa de equilibrio y monitoreo transversal: HAProxy distribuye tráfico hacia `unwheels-auth` y `healthmon` ejecuta heartbeat y observabilidad sobre los microservicios. |
...
| **Datos**                  | `auth-db`, `routes-db`, `chat-db`, `notifications-db`, `search-db`, `search-redis`               | PostgreSQL, MongoDB ×3, PostgreSQL+PostGIS, Redis 7 | Persistencia por servicio y caché auxiliar; `search-redis` complementa la capa de datos para consultas y lectura acelerada.                                             |

**Estructura interna de capas — Tier de Balanceo:**

| Componente   | Capa                | Responsabilidad                                                                    |
| ------------ | ------------------- | ---------------------------------------------------------------------------------- |
| `loggueo-lb` | Load Balancer       | Distribuye el tráfico de autenticación entre las tres réplicas de `unwheels-auth`. |
| `healthmon`  | Heartbeat / Monitor | Ejecuta probes periódicos y emite eventos de salud para los microservicios.        |
...
![Escenario 2 — Cache-Aside / búsqueda de rutas](./imgs/performance_report/scenario2_team_pattern_performance.png)
