...
# Prototipo 4 — Atributos De Calidad
...
- **Source:** Servicio de autenticación (`loggueo_service`).
- **Stimulus:** Fallo de una réplica (`loggueo_1`) por caída del proceso o indisponibilidad de red.
- **Artifact:** Tres réplicas activas (`loggueo_1`, `loggueo_2`, `loggueo_3`) desplegadas en Docker Compose.
- **Environment:** Operación normal del sistema en Docker Desktop.
...
- **Response measure:** El servicio permanece disponible sin interrupción perceptible para el usuario (0 errores visibles de login en la demo con 1 de 3 réplicas caídas).
...
![Escenario 2 — Service Discovery Pattern](./imgs/reliability/ESCENARIO_RELIABILITY_2.png)

- **Source:** Servicio de autenticación y su punto de entrada lógico.
- **Stimulus:** Petición de autenticación entrante mientras una réplica no está disponible.
- **Artifact:** DNS estable `loggueo-service` → HAProxy (`loggueo-lb`) con `option httpchk GET /readyz` cada 5 s.
- **Environment:** Operación normal; réplicas registradas estáticamente en el backend de HAProxy.
- **Response:** El mecanismo de discovery (health-informed routing) excluye la instancia caída del pool activo y enruta hacia réplicas `passing`.
- **Response measure:** La autenticación se completa con éxito sin que el cliente conozca qué réplica atendió la petición.
...
_Pendiente: completar táctica asociada al Escenario 3 (Cluster)._
...
_Pendiente: documentar aplicación del Cluster Pattern en el sistema._
...
| Escenario 3 – Cluster | Cluster Pattern | _Pendiente._ |
