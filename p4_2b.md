...
- **Stimulus:** 300 usuarios virtuales concurrentes inician peticiones `POST /api/auth/login` en un intervalo de 30 segundos (~3.540 requests), todas dirigidas al único punto de entrada `https://localhost:8080` (`rp-web` → `api-gateway`).
- **Environment:** Operación normal en inicio de semestre. Tres réplicas de `loggueo_service` (`loggueo_1`, `loggueo_2`, `loggueo_3`) activas y saludables detrás de HAProxy (`loggueo-lb`). PostgreSQL compartida entre las tres instancias. Pruebas en Docker Desktop (12 vCPU); WAF y rate limiting deshabilitados para aislar la capacidad del balanceador.
- **Response:** HAProxy distribuye las peticiones entre las tres réplicas mediante algoritmo **least-conn**, excluyendo instancias que no superan el health check activo `GET /readyz`.
- **Response measure:** En el punto de rodilla (300 VUs concurrentes): rendimiento pico **118 req/s**, latencia p95 **4.800 ms**, tasa de error **2 %**. Con ≤ 250 VUs: latencia media **~550 ms** y **0 %** de fallos.
...
**Nota:** La medida de respuesta se obtuvo de la simulación k6 modelada a partir de pruebas parciales y saturación de CPU del host; el escenario demuestra que el **Load Balancer Pattern** distribuye la carga antes de que bcrypt sature los workers de cada réplica.
