...
- **Source:** 250–300 usuarios legítimos intentando autenticarse concurrentemente.
- **Stimulus:** Ráfagas de `POST /api/auth/login` generadas por k6 en ventanas de 30 s.
- **Artifact:** Componente `loggueo-service` (HAProxy + `loggueo_1/2/3`) detrás del `api-gateway`.
- **Environment:** Operación normal bajo tráfico alto; WAF y rate limiting deshabilitados para la prueba.
- **Response:** HAProxy distribuye las peticiones entre réplicas sanas; el sistema procesa logins hasta el punto de rodilla.
- **Response measure:** Con ≤ 250 VUs, todas las peticiones se procesan con 0 % de fallos; a 300 VUs el rendimiento alcanza 118 req/s con degradación controlada (2 % de fallos).
...
**Nota:** El tiempo reportado refleja el comportamiento del sistema bajo las condiciones de prueba; el escenario demuestra que el **Load Balancer Pattern** eleva el throughput antes de que bcrypt sature los workers de cada réplica.
