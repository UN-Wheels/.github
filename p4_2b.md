...
| **Heartbeat Pattern** (equipo)      | `healthmon` hace polling cada 15s con umbral de 3 fallos a servicios **sin LB propio** (chat, routes, notifications, search, liveness del LB). No repite httpchk de réplicas auth — eso lo hace HAProxy. Docker healthcheck usa `/healthz` (liveness) en réplicas.                                                                                                                                                                                         |
| **Service Discovery Pattern**       | DNS estable `loggueo-service` → HAProxy (`loggueo-lb`) con pool de 3 réplicas. `option httpchk GET /readyz` (cada 5 s) es la **única fuente de verdad** para enrutamiento; excluye instancias caídas. Heartbeat observa servicios sin LB, sin duplicar checks a réplicas auth.                                                                                                                                                                             |
| **Load Balancer Pattern**           | HAProxy balancea `loggueo_1/2/3` con Round-Robin (configurable a Least-Conn o Source IP). El api-gateway resuelve una sola URL; el LB excluye réplicas unhealthy vía httpchk.                                                                                                                                                                                                                                                                              |
| **Despliegue Multi-nodo (Clúster)** | la infraestructura lógica se distribuye en un clúster multi-nodo gestionado por Docker Swarm sobre una red local. La suite perimetral, el API Gateway, los servicios de soporte y el Tier de Datos (Stateful) se anclan de forma estricta al Nodo Manager. En contraste, el microservicio crítico de rutas `unwheels-routes` implementa el Cluster Pattern en redundancia activa dispersando de forma asimétrica una de sus réplicas hacia un Nodo Worker. |
...
- **Stimulus:** Intentos de realizar _sniffing_, _man-in-the-middle_ o _downgrade attacks_ durante el intercambio de autenticación, búsqueda de rutas, reserva de cupos o mensajería en tiempo real con el frontend.
...
### Performance scenarios in prototype 4

#### Pruebas de carga con k6 (Docker Desktop)

Las pruebas de rendimiento se ejecutaron sobre el stack completo de UN Wheels desplegado en **Docker Desktop** sobre un único host, accediendo vía `https://localhost:8080` (reverse proxy `rp-web`). El rate limiting de Nginx se deshabilitó temporalmente para observar la capacidad real del stack de aplicación.

**Infraestructura de prueba:**

| Componente               | Especificación                                                                                  |
| ------------------------ | ----------------------------------------------------------------------------------------------- |
| **Orquestación**         | Docker Compose (`FrontEnd/docker-compose.yml`)                                                  |
| **Balanceador de auth**  | HAProxy (`loggueo-lb`) con 3 réplicas `loggueo_1/2/3`                                           |
| **Herramienta de carga** | k6                                                                                              |
| **Endpoints evaluados**  | `POST /api/auth/login` (aislado) y journey completo: login → `/me` → búsqueda de rutas → logout |

![Pruebas de carga k6 — configuración del entorno](./imgs/performance_report/performance_scenario_env.png)

Las pruebas aisladas de login y el **Full Session Journey** permitieron ubicar el _knee point_ del sistema y confirmar que el cuello de botella se concentra en `loggueo-service` (autenticación con bcrypt + PostgreSQL), mientras que `route-search-service` (Go) mantiene latencias estables incluso bajo carga extrema.

#### Full Session Journey — Resumen de métricas

![Full Session Journey — knee point](./imgs/performance_report/knee_point_chart_5.png)

Bajo estas condiciones, el _knee point_ se ubicó en **100 VUs concurrentes**, con las siguientes métricas promedio en esa zona:

- Latencia media del journey: ~2.414 ms (+5.573 % respecto a la zona estable)
- Tasa de error global: ~12 %
- Throughput efectivo de login: degradado por saturación del pool de conexiones SQLAlchemy

Para comparar, con **50 VUs** (zona estable) el sistema reportó:

- Latencia media del journey: ~64 ms
- Tasa de error: 0 %
- Throughput de login: ~39 req/s (Test 2 aislado)

#### Prueba de Load Balancer bajo réplicas (escenario complementario)

![Load Balancer — distribución entre réplicas](./imgs/performance_report/lb_replicas_distribution.png)

_Pendiente: imagen de evidencia de distribución Round-Robin entre `loggueo_1/2/3` bajo carga sostenida._

---

### Escenario 1 – Rendimiento del Servicio de Autenticación (Load Balancer Pattern)

- **Source:** 50–100 usuarios legítimos intentando autenticarse concurrentemente.
- **Stimulus:** Ráfagas de peticiones `POST /api/auth/login` distribuidas por k6 dentro de ventanas de 30 s a 3 min.
- **Artifact:** Componente `loggueo-service` (HAProxy + pool `loggueo_1/2/3`) detrás del `api-gateway`.
- **Environment:** Operación normal bajo tráfico alto; rate limiting deshabilitado para la prueba.
- **Response:** El balanceador distribuye las peticiones entre las tres réplicas; el sistema procesa autenticaciones exitosas hasta el _knee point_ (~100 VUs en journey completo).
- **Response measure:** Con ≤ 50 VUs concurrentes, todas las peticiones de login se procesan con latencia media ≤ 400 ms y 0 % de fallos.

<div align="center">

![Escenario 1 — Rendimiento del servicio de autenticación](./imgs/performance_report/scenario1_authentication_performance.png)

</div>

**Nota:** El tiempo reportado en la medida de respuesta corresponde al comportamiento actual del sistema bajo las condiciones de prueba; el escenario busca demostrar que el **Load Balancer Pattern** sostiene carga moderada antes de que el dominio de autenticación alcance saturación.

---

### Escenario 2 – Rendimiento del Patrón del Equipo (Cache-Aside en Búsqueda)

Durante el inicio de jornada académica (7:00–8:00 AM), un alto volumen de estudiantes busca rutas simultáneamente con parámetros geográficos similares (mismos polígonos de origen/destino). Sin caché, cada una de estas búsquedas repetitivas ejecuta una query geoespacial costosa sobre PostGIS, saturando la base de datos y degradando la latencia para todos los usuarios. El patrón Cache-Aside permite que las búsquedas equivalentes se resuelvan desde Redis en memoria, reduciendo la carga sobre PostGIS y mejorando el tiempo de respuesta en escenarios de alta concurrencia con consultas repetitivas.

![Representación gráfica](./imgs/performance_report/scenario2_team_pattern_performance.png)

_Descripción del Escenario_

- **Source:** Ráfaga de búsquedas geoespaciales simultáneas generada por estudiantes de la UNAL buscando rutas disponibles durante el inicio de jornada académica (7:00–8:00 AM).
- **Stimulus:** 200 requests de búsqueda de rutas con parámetros similares (mismos polígonos de origen/destino) llegan al search-service en un intervalo de 60 segundos, cada una ejecutando una query geoespacial sobre PostGIS.
- **Artifact:** Servicio `unwheels-search` + Redis (caché) + PostGIS (`route-search-postgres`).
- **Environment:** Producción, hora pico académica, PostGIS bajo carga sostenida por el syncer de rutas activo simultáneamente.
- **Response:** El search-service consulta Redis antes de ejecutar cualquier query a PostGIS. Las búsquedas con parámetros idénticos o equivalentes a consultas recientes se resuelven directamente desde Redis con latencia de memoria. Solo las búsquedas con parámetros nuevos o cuyo TTL haya expirado alcanzan PostGIS. El resultado de cada cache miss se escribe en Redis con un TTL de 120 segundos antes de retornarse al cliente.
- **Response measure:** Tiempo de respuesta promedio ≤ 150 ms para cache hits vs ≤ 900 ms para cache misses directos a PostGIS; tasa de cache hits ≥ 60% durante hora pico; carga de queries sobre PostGIS reducida en ≥ 60% respecto al escenario sin caché; 0 inconsistencias de datos (el syncer invalida entradas de caché al detectar cambios en rutas).

**Nota:** El tiempo reportado en la medida de respuesta corresponde al comportamiento esperado del sistema bajo las condiciones de prueba; el escenario busca demostrar que el **Cache-Aside Pattern** reduce la latencia y la carga sobre la base de datos en escenarios de consultas repetitivas bajo alta concurrencia.

#### Requisitos del Patrón

Para garantizar la operatividad del Cache-Aside en UN Wheels, se establecen los siguientes requerimientos:

| Requisito | Descripción |
| --- | --- |
| **Redis como store de caché** | Contenedor Redis 7 añadido a la zona DATA del `docker-compose.yml`, accesible únicamente desde la red interna `data`. No expone puertos al host. Persiste datos con AOF (Append-Only File) para sobrevivir reinicios sin pérdida de entradas calientes. |
| **Clave de caché determinista** | El search-service construye la clave de caché a partir de los parámetros de búsqueda normalizados (coordenadas redondeadas a 4 decimales, radio de búsqueda, filtros activos) usando un hash SHA-256. Búsquedas equivalentes con representaciones ligeramente distintas golpean la misma entrada de caché. |
| **TTL configurable por entorno** | El TTL de cada entrada se controla mediante la variable de entorno `CACHE_TTL_SECONDS` (default: 120). Permite ajustar el balance entre frescura de datos y tasa de hits sin recompilar el servicio. |
| **Invalidación activa por el syncer** | El syncer interno del search-service (que sincroniza rutas desde `routes-reservations-service` cada 60 segundos) invalida las entradas de caché afectadas cuando detecta rutas nuevas, modificadas o eliminadas. Evita que usuarios reciban resultados de búsqueda con rutas ya desactivadas. |
| **Métricas de caché observables** | El search-service registra en stdout un log estructurado JSON por cada operación de caché con los campos `event` (hit/miss/invalidated), `key`, `latency_ms` y `timestamp`. Permite calcular la tasa de hits real en producción. |
| **Degradación elegante** | Si Redis no está disponible (timeout de conexión > 200 ms), el search-service omite la caché silenciosamente y sirve la respuesta directamente desde PostGIS, registrando el evento como `cache_unavailable`. El servicio nunca falla por indisponibilidad de la caché. |

#### Tecnologías Seleccionadas y Justificación

| Componente | Tecnologías | Justificación |
| --- | --- | --- |
| **unwheels-search (Cache-Aside)** | Go `go-redis/v9`, Redis 7 Alpine, Docker healthcheck, variable `CACHE_TTL_SECONDS` | Se elige Redis sobre alternativas en memoria (`sync.Map`, `bigcache`) porque es un componente externo al proceso: la caché sobrevive reinicios del contenedor del search-service y puede ser compartida si en el futuro se escala a múltiples réplicas del servicio. La librería `go-redis/v9` es la cliente oficial para Go, soporta context con timeout y permite implementar el patrón cache-aside en menos de 30 líneas sin dependencias adicionales. |

Redis se añade a la zona DATA del docker-compose porque es exclusivamente un almacén de datos del search-service; ningún otro microservicio lo usa ni debe acceder a él.

---

### Tácticas arquitectónicas aplicadas (Rendimiento)

#### 1. Maintain Multiple Copies of Computations

**Descripción:** Esta táctica mejora el rendimiento mediante escalamiento horizontal, creando copias idénticas del mismo componente para distribuir la carga de procesamiento y reducir el tiempo de respuesta promedio.

**Aplicación:** Se despliegan tres réplicas del microservicio de autenticación (`loggueo_1`, `loggueo_2`, `loggueo_3`) detrás de HAProxy (`loggueo-lb`). Las peticiones de login se reparten entre instancias disponibles, aliviando la contención en un solo proceso Python/uvicorn.

**Patrón asociado:** Load Balancer Pattern, Service Replication Pattern (Hot Spare).

#### 2. Introduce Concurrency

**Descripción:** Permite que múltiples peticiones se atiendan en paralelo cuando el componente lo soporta, incrementando el throughput bajo demanda.

**Aplicación:** El pool de réplicas de auth y la arquitectura de microservicios (p. ej. `route-search-service` en Go) permiten procesar búsquedas y autenticaciones de forma concurrente en procesos separados.

**Patrón asociado:** Load Balancer Pattern.

#### 3. Maintain Multiple Copies of Data (Caching)

**Descripción:** Esta táctica almacena copias de datos frecuentemente consultados en un almacén de acceso rápido (caché en memoria), de modo que las lecturas repetitivas no necesiten recurrir al almacenamiento persistente. Reduce la latencia de respuesta y la carga sobre la base de datos bajo alta concurrencia.

**Aplicación:** El servicio `unwheels-search` implementa el patrón Cache-Aside con Redis: antes de ejecutar una query geoespacial sobre PostGIS, consulta Redis con una clave determinista (SHA-256 de los parámetros normalizados). Los cache hits se resuelven en ≤ 150 ms frente a ≤ 900 ms de un miss directo a PostGIS, con un TTL de 120 segundos y invalidación activa por el syncer de rutas.

**Patrón asociado:** Cache-Aside Pattern.

---

### Patrones arquitectónicos aplicados (Rendimiento)

#### 1. Load Balancer Pattern

UN Wheels aplica el **Load Balancer Pattern** en el dominio de autenticación. El componente `loggueo-lb` (HAProxy) recibe todas las peticiones del `api-gateway` dirigidas a `loggueo-service:8000` y las distribuye entre `loggueo_1`, `loggueo_2` y `loggueo_3` mediante Round-Robin (configurable a Least-Conn o Source IP). El patrón mejora el throughput y evita que una sola instancia concentre toda la carga de login.

#### 2. Service Replication Pattern (Hot Spare)

UN Wheels replica el servicio de autenticación en **tres instancias activas simultáneamente** (Hot Spare). Todas están en ejecución y listas para recibir tráfico; no existe réplica fría apagada. HAProxy enruta solo a réplicas que superan el health check `GET /readyz`.

#### 3. Cache-Aside Pattern

UN Wheels aplica el **Cache-Aside Pattern** en el dominio de búsqueda de rutas. El componente `unwheels-search` (Go) consulta Redis antes de ejecutar cualquier query geoespacial sobre PostGIS. Las búsquedas con parámetros idénticos o equivalentes se resuelven directamente desde la caché en memoria (cache hit), mientras que solo las consultas nuevas o con TTL expirado alcanzan la base de datos (cache miss). El resultado de cada miss se escribe en Redis con un TTL configurable (default 120 s). El syncer interno invalida entradas de caché al detectar cambios en rutas, y ante indisponibilidad de Redis el servicio degrada elegantemente sirviendo directamente desde PostGIS.

---

### Relación entre Patrones Aplicados y Escenarios (Rendimiento)

| **Escenario** | **Patrón** | **Explicación** |
| --- | --- | --- |
| Escenario 1 – Rendimiento del Servicio de Autenticación | Load Balancer Pattern | HAProxy distribuye las peticiones de login entre tres réplicas, elevando el throughput y retrasando la saturación de un solo nodo. |
| Escenario 1 – Rendimiento del Servicio de Autenticación | Service Replication Pattern | Las tres réplicas activas de `loggueo_service` procesan autenticaciones en paralelo detrás del mismo punto de entrada lógico. |
| Escenario 2 – Rendimiento del Patrón del Equipo | Cache-Aside Pattern | El search-service consulta Redis antes de PostGIS; las búsquedas repetitivas se resuelven desde caché en ≤150 ms, reduciendo la carga sobre la base de datos en ≥60% durante hora pico. |
...
![Escenario 1 — Replication Pattern (Hot Spare)](./imgs/reliability/ESCENARIO_RELIABILITY_1.png)
...
![Escenario 2 — Service Discovery Pattern](./imgs/Service_Discovery_Pattern.png)
...
```powershell
docker exec loggueo_1 python -c "import urllib.request; print(urllib.request.urlopen('http://127.0.0.1:8000/readyz').read())"
docker exec loggueo-lb wget -qO- http://127.0.0.1:8000/healthz
```
...
![Escenario 3 — Cluster Pattern](./imgs/reliability/ESCENARIO_RELIABILITY_3.png)
...
- **Response measure:** Fallo detectado en ≤ 3 × intervalo de check (45 s); el operador puede consultar el estado centralizado sin afectar el tráfico de producción.

**Servicios monitoreados por Heartbeat:**

| Servicio                | URL                                              |
| ----------------------- | ------------------------------------------------ |
| `loggueo-lb`            | `http://loggueo-service:8000/healthz`            |
| `chat-service`          | `http://chat-service:3001/health`                |
| `routes-service`        | `http://routes-reservations-service:4000/health` |
| `notifications-service` | `http://notifications-service:3002/health`       |
| `search-service`        | `http://route-search-service:6000/health`        |
...
Las réplicas `loggueo_1/2/3` están activas al mismo tiempo. Ante el fallo de una, las demás continúan atendiendo login sin activación manual de una réplica fría.

#### 3. Service Discovery Pattern

El `api-gateway` resuelve el nombre lógico `loggueo-service:8000`. HAProxy es la **única fuente de verdad** para enrutamiento: `option httpchk GET /readyz` cada 5 s, `fall 3` / `rise 2`. No se utiliza Consul; es discovery server-side simplificado para Docker Compose.
...
| Escenario 4 – Heartbeat               | Heartbeat Pattern               | `healthmon` detecta y registra fallos en servicios sin LB sin controlar el tráfico. |

---

### Pruebas de Verificación (Fiabilidad)

#### 1. Service Discovery — réplica caída

```powershell
cd FrontEnd
docker pause loggueo_1
# Probar login → debe seguir OK
docker unpause loggueo_1
```

![Service Discovery — demo réplica caída](./imgs/reliability/sd_demo_replica_down.png)

#### 2. Heartbeat — estado centralizado

```powershell
docker exec health-monitor wget -qO- http://127.0.0.1:9090/status
```

![Heartbeat — healthmon status](./imgs/reliability/heartbeat_status.png)

#### 3. Cluster Pattern

_Pendiente: evidencia de prueba._

![Cluster Pattern — verificación](./imgs/reliability/cluster_verification.png)

#### 4. Replication Pattern

Evidencia del mecanismo de failover del Replication Pattern

![Replication Pattern — verificación](./imgs/replication1.jpeg)

La imagen muestra los logs del contenedor `loggueo_3` durante una operación de lectura. El mensaje [DB] read routed to REPLICA confirma que la solicitud fue enrutada automáticamente hacia el endpoint de réplica de lectura en Neon, dejando el nodo primario exclusivamente para operaciones de escritura. Este comportamiento implementa el Replication Pattern: las lecturas se distribuyen a la réplica para reducir la carga sobre el primario, y en caso de que la réplica no esté disponible, el sistema cae automáticamente al nodo primario sin interrumpir el servicio, garantizando así la disponibilidad continua del microservicio de autenticación.

![Replication Pattern — verificación](./imgs/replication2.jpeg)

La imagen muestra los logs obtenidos con el comando docker compose logs -f `loggueo_1`, `loggueo_2`, `loggueo_3` durante una operación de lectura con la réplica de Neon simulada como no disponible. El mensaje [DB] REPLICA unavailable indica que
el intento de conexión al endpoint de réplica falló con un error de resolución de nombre (Name or service not known), lo que representa una caída total del nodo de lectura. Inmediatamente después, el sistema ejecuta el fallback automático y el mensaje [DB] read routed to PRIMARY (fallback) confirma que la solicitud fue redirigida al nodo primario de forma transparente. El request GET /api/v1/vehicles/ completó con 200 OK, demostrando que el microservicio de autenticación mantuvo disponibilidad completa sin intervención manual ni error visible para el usuario final, que es el objetivo central del Replication Pattern aplicado a alta disponibilidad.

---

## 7. Atributos de Calidad — Interoperabilidad

Escenario de interoperabilidad del Prototipo 4.

### Escenario – Interoperabilidad (JWT Compartido entre Microservicios)

![Escenario de interoperabilidad](./imgs/interoperability/ESCENARIO_INTEROPERABILITY.png)
...
_Pendiente: documentar escenario de interoperabilidad (validación de JWT compartido vía API Gateway entre auth, routes, chat y notifications)._

---

### Tácticas arquitectónicas aplicadas (Interoperabilidad)
...
### Patrones arquitectónicos aplicados (Interoperabilidad)
...
### Relación entre Patrones Aplicados y Escenarios (Interoperabilidad)

| **Escenario**                 | **Patrón**                         | **Explicación** |
| ----------------------------- | ---------------------------------- | --------------- |
| Escenario – Interoperabilidad | API Gateway Pattern                | _Pendiente._    |
| Escenario – Interoperabilidad | Token-Based Authentication Pattern | _Pendiente._    |
